...
def _obtener_lista_credenciales():
    ...
    # 3. Carga desde archivos .json locales en la raíz del proyecto
    archivos_json = list(Path(".").glob("*.json"))
    for ruta_json in archivos_json:
        try:
            contenido = _json.loads(ruta_json.read_text(encoding="utf-8"))
...
def obtener_clientes_gspread():
    ...
    if not lista_info:
        return [], (
            ...
            "[gcp_service_account] en Secrets; en local coloca los archivos .json de las cuentas "
            "de servicio en la misma carpeta del script."
        )
...
_CLAVES_FILTROS = [
    "filtro_busqueda_dependencia", "filtro_estados", "filtro_dependencias",
    "filtro_ejercicios", "filtro_estatus",
]

if not df.empty:
    if st.sidebar.button("🧹 Restablecer filtros", use_container_width=True):
        for _clave in _CLAVES_FILTROS:
    ...
    # --- Búsqueda libre por nombre de Dependencia ---
    busqueda_dependencia = st.sidebar.text_input(
        "🔎 Buscar Dependencia:",
        value=st.session_state.get("filtro_busqueda_dependencia", ""),
        key="filtro_busqueda_dependencia",
        placeholder="Escribe parte del nombre...",
    )
    df_filtrado = df.copy()
    if busqueda_dependencia.strip() and col_dependencia in df_filtrado.columns:
        df_filtrado = df_filtrado[
            df_filtrado[col_dependencia]
            .astype(str)
            .str.contains(busqueda_dependencia.strip(), case=False, na=False, regex=False)
        ]

    # --- Filtro por Estado (multiselección) ---
    estados_disponibles = sorted(df_filtrado[col_estado].dropna().unique().tolist()) if col_estado in df_filtrado.columns else []
    estados_seleccionados = st.sidebar.multiselect(
        "Estado:", options=estados_disponibles, key="filtro_estados",
        placeholder="Todos los Estados",
    )
    df_filtrado_estado = (
        df_filtrado[df_filtrado[col_estado].isin(estados_seleccionados)]
        if estados_seleccionados else df_filtrado
    )

    # --- Filtro por Dependencia (multiselección, en cascada según Estado) ---
    dependencias_disponibles = sorted(df_filtrado_estado[col_dependencia].dropna().unique().tolist()) if col_dependencia in df_filtrado_estado.columns else []
    # Evita que queden seleccionadas dependencias que ya no aplican tras cambiar el Estado
    dependencias_previas = [d for d in st.session_state.get("filtro_dependencias", []) if d in dependencias_disponibles]
    if dependencias_previas != st.session_state.get("filtro_dependencias", []):
        st.session_state["filtro_dependencias"] = dependencias_previas
    dependencias_seleccionadas = st.sidebar.multiselect(
        "Dependencia:", options=dependencias_disponibles, key="filtro_dependencias",
        placeholder="Todas las Dependencias",
    )
    df = (
        df_filtrado_estado[df_filtrado_estado[col_dependencia].isin(dependencias_seleccionadas)]
        if dependencias_seleccionadas else df_filtrado_estado
    )

if not df.empty and "Ejercicio Fiscal" in df.columns:
    ejercicios_disponibles = sorted(df["Ejercicio Fiscal"].dropna().unique().tolist())
    ejercicios_seleccionados = st.sidebar.multiselect(
        "Ejercicio Fiscal:", options=ejercicios_disponibles, key="filtro_ejercicios",
        placeholder="Todos los Ejercicios",
    )
    if ejercicios_seleccionados:
        df = df[df["Ejercicio Fiscal"].isin(ejercicios_seleccionados)]
...
if not df.empty and "Estatus" in df.columns:
    ...
    estatus_seleccionados = st.sidebar.multiselect(
        "Estatus:", options=estatus_disponibles, key="filtro_estatus",
        placeholder="Todos los Estatus",
    )
    if estatus_seleccionados:
        df = df[df["Estatus"].isin(estatus_seleccionados)]

if TOTAL_REGISTROS_ORIGINAL:
    _filtros_activos = any(st.session_state.get(_c) for _c in _CLAVES_FILTROS)
    if _filtros_activos:
        st.sidebar.caption(f"📊 Mostrando **{len(df):,}** de **{TOTAL_REGISTROS_ORIGINAL:,}** registros.")
    if df.empty:
        st.sidebar.warning("No hay registros que coincidan con los filtros seleccionados.")
...
with pestana_contratos:
    ...
    ruta_credenciales = os.path.join(
        os.path.dirname(__file__), "credenciales.json"
    )
    ...
    def _extraer_id_carpeta_drive(url_o_id: str) -> str:
        """Acepta tanto una URL de Drive como un ID directo y devuelve solo el ID."""
    ...
    FOLDER_URL_RAIZ = "https://drive.google.com/drive/folders/1eVDyjip3olampBF8ia_tAvCqtWkaqRLY?usp=drive_link"
    FOLDER_ID_RAIZ = _extraer_id_carpeta_drive(FOLDER_URL_RAIZ)

    @st.cache_resource
    def get_drive_service():
        """Autentica y crea el cliente de la API de Google Drive."""
        # Lee las credenciales del archivo JSON local o de st.secrets
        try:
            if "google_credentials" in st.secrets:
                # Si usas Streamlit Community Cloud (Secrets)
                creds_dict = json.loads(st.secrets["google_credentials"])
                creds = Credentials.from_service_account_info(
                    creds_dict, scopes=SCOPES
                )
            else:
                # Si estás ejecutando localmente con credenciales.json
                creds = Credentials.from_service_account_file(
            ...
            st.error(
                f"Error al autenticar con Google Drive API: {e}. Revisa tu"
    ...
    if not FOLDER_ID_RAIZ:
        st.warning(
            "⚠️ Por favor configura tu `FOLDER_URL_RAIZ` en el código con el enlace o"
