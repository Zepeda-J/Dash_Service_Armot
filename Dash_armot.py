...
with pestana_contratos:
    ...
    st.write("Aquí puedes gestionar los contratos asociados a los servicios de Armot.")
        ...
        # 2. Base de datos de usuarios con Roles asignados ('admin' o 'usuario')
    USUARIOS = {
    ...
    # 3. Control de sesión (Session State)
    ...
    def mostrar_login():
        ...
        with col2:
            ...
            st.write("Ingresa tus credenciales para acceder a la nube de documentos.")
            ...
            with st.form("form_login"):
                ...
                if submit:
                    ...
                    if usuario in USUARIOS and USUARIOS[usuario]["clave"] == password:
                        st.session_state["autenticado"] = True
                        st.session_state["usuario_actual"] = usuario
                        st.session_state["rol_actual"] = USUARIOS[usuario]["rol"]  # Guardar el rol del usuario
                        st.success("¡Inicio de sesión exitoso!")
                        st.rerun()
                    else:
                        st.error("Usuario o contraseña incorrectos.")

    # --- NUBE DE ARCHIVOS Y GESTIÓN ---
    def mostrar_nube():
        ...
        st.title("☁️ Mi Nube de Documentos")
    
        # Ruta base de la carpeta contenedora
        BASE_DIR = os.path.join(os.getcwd(), "mi_nube")
        ...
        with st.sidebar.expander("➕ Crear Nueva Carpeta"):
            ...
            if st.button("Crear Carpeta"):
                if nombre_nueva_carpeta.strip() != "":
                    nombre_carpeta_seguro = Path(nombre_nueva_carpeta.strip()).name
                    nueva_ruta = os.path.join(BASE_DIR, nombre_carpeta_seguro)
                    if not os.path.exists(nueva_ruta):
                        os.makedirs(nueva_ruta)
                        st.success(f"Carpeta '{nombre_carpeta_seguro}' creada.")
                        st.rerun()
                    else:
        ...
        if rol == "admin":
            if subcarpetas_existentes:
                with st.sidebar.expander("🗑️ Eliminar Carpeta (Solo Admin)"):
                    carpeta_a_borrar = st.selectbox("Selecciona carpeta a borrar:", subcarpetas_existentes)
                    confirmar_borrado = st.checkbox("Confirmar eliminación")
                    if st.button("Eliminar carpeta"):
                        if confirmar_borrado:
                            ruta_borrar = os.path.join(BASE_DIR, carpeta_a_borrar)
        ...
        opciones_carpetas = ["Raíz"] + subcarpetas_existentes
        carpeta_seleccionada = st.sidebar.selectbox("Carpeta activa:", opciones_carpetas)

        ruta_actual = BASE_DIR if carpeta_seleccionada == "Raíz" else os.path.join(BASE_DIR, carpeta_seleccionada)
        ...
        if archivo_subido is not None:
            ...
            ruta_destino = os.path.join(ruta_actual, nombre_archivo_seguro)
            with open(ruta_destino, "wb") as f:
                f.write(archivo_subido.getbuffer())
            st.sidebar.success(f"Guardado: '{nombre_archivo_seguro}'")
            st.rerun()

        # --- 4. EXPLORADOR DE ARCHIVOS Y ELIMINACIÓN RESTRINGIDA ---
        st.subheader(f"📂 Contenido de: `{carpeta_seleccionada}`")
        archivos = [
            f for f in os.listdir(ruta_actual) 
            if os.path.isfile(os.path.join(ruta_actual, f)) and f.endswith(('.pdf', '.xlsx', '.docx'))
        ]
        ...
        if not archivos:
            ...
            for archivo in archivos:
                ...
                with st.expander(f"{icono} — {archivo}"):
                    ...
                    if rol == "admin":
                        if col2.button("🗑️ Eliminar", key=f"del_{archivo}"):
