...
with col_centro:
    ...
    with col_img:
        st.image("Armot_Color.png", width="stretch")
    ...
    if prompt_ia := st.chat_input("¿Qué deseas saber de los datos?"):
        ...
        with st.chat_message("assistant"):
            if client_gemini is None:
                st.error("⚠️ El cliente de Gemini no está inicializado correctamente. Revisa tu API Key.")
...
st.html(
    ...
    <style>
        ...
        /* Estilos del botón flotante/link de WhatsApp */
        .wa-link {
            display: inline-flex;
            align-items: center;
            gap: 6px;
        ...
        with st.container(border=True):
            ...
            with st.expander("📂 Menú de Información Completa", expanded=False):
                ...
                with tab_contacto:
                    ...
                    # Renders link to WhatsApp with SVG icon
                    if link_wa:
                        st.markdown(
            ...
            with st.expander("📂 Menú de Información Completa", expanded=False):
                ...
                with col_btn2:
                    ...
                    st.download_button(
                        ...
                        mime="image/png", key=f"download_admin_{admin['id']}_{idx}", width="stretch",
                    )

                col_btn3, col_btn4 = st.columns(2)
                with col_btn3:
                    if st.button("✏️ Editar", key=f"edit_admin_{admin['id']}_{idx}", width="stretch"):
                        dialogo_editar_admin(admin["id"])
                with col_btn4:
                    if st.session_state.confirmar_borrado_admin == admin["id"]:
                        if st.button("⚠️ Confirmar", key=f"confirm_del_admin_{admin['id']}_{idx}", width="stretch", type="primary"):
                        ...
                        if st.button("🗑️ Eliminar", key=f"del_admin_{admin['id']}_{idx}", width="stretch"):
...
with pestana_facturacion:
    st.subheader("🧾 Seguimiento de Facturación")
    st.info("Módulo de seguimiento de facturación en proceso de integración.")
