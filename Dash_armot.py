...
# --- IMPORTACION DE LA LIBRERIA OFICIAL GEMINI ---
...
# =========================================================================
GEMINI_API_KEY = "AQ.Ab8RN6K78w2dWYRFl4NSIhJfoGZAIrMAuM3BsXEaQq30GunJCQ"
    ...
    if prompt_ia := st.chat_input("¿Qué deseas saber de los datos?"):
        ...
        with st.chat_message("assistant"):
            if not client_gemini:
                mensaje_error = "⚠️ No se pudo inicializar el cliente de Gemini. Verifica tu conexión."
                st.error(mensaje_error)
                st.session_state.chat_history.append({"role": "assistant", "content": mensaje_error})
            elif df.empty:
                mensaje_error = "⚠️ No hay datos cargados en 'Servicio.xlsx' para analizar."
                st.error(mensaje_error)
                st.session_state.chat_history.append({"role": "assistant", "content": mensaje_error})
            else:
                with st.spinner("Analizando información..."):
                    contexto_datos = (
                        f"Eres un asistente analítico experto para la empresa Armot.\n"
                        f"A continuación tienes un resumen de los datos de 'Servicio.xlsx':\n"
                        f"- Número de Dependencias Únicas: {df['Dependencia'].nunique() if 'Dependencia' in df.columns else 0}\n"
                        f"- Total de Unidades: {df['N° de Unidades'].sum() if 'N° de Unidades' in df.columns else 0}\n"
                        f"- Elementos Mínimos: {df['Elementos minimos'].sum() if 'Elementos minimos' in df.columns else 0}\n"
                        f"- Elementos Máximos: {df['Elementos máximos'].sum() if 'Elementos máximos' in df.columns else 0}\n"
                        f"- Monto Mínimo con IVA Global: ${df['Monto mínimo con IVA'].sum():,.2f}\n"
                        f"- Monto Máximo con IVA Global: ${df['Monto máximo con IVA'].sum():,.2f}\n\n"
                        f"Muestra de filas principales:\n{df.head(15).to_string()}\n\n"
                        f"Responde de forma clara y ejecutiva a: {prompt_ia}"
                    )
                    
                    try:
                        respuesta = client_gemini.models.generate_content(
                            model='gemini-3.6-flash',
                            contents=contexto_datos,
                        )
