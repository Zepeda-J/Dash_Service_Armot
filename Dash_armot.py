...
# --- LIBRERÍAS DE GOOGLE DRIVE (opcionales: si no están instaladas, la pestaña
# de Contratos mostrará un aviso en vez de tumbar toda la app) ---
try:
...
# ============================================================================
USUARIOS = {
    "admin": {"clave": "Alex_armot_2026", "rol": "admin"},
...
# =========================================================================
# CONFIGURACIÓN E INTEGRACIÓN CON GOOGLE DRIVE
# =========================================================================
...
def obtener_servicio_drive():
    """Crea el cliente de la API de Google Drive a partir de una cuenta de
    servicio configurada en st.secrets. Devuelve (servicio, error)."""
    if not DRIVE_LIBS_DISPONIBLES:
        return None, "Las librerías de Google Drive no están instaladas (google-api-python-client, google-auth)."
    if "gcp_service_account" not in st.secrets:
        return None, "Falta configurar el secreto 'gcp_service_account' en Streamlit."
    if "DRIVE_ROOT_FOLDER_ID" not in st.secrets:
        return None, "Falta configurar el secreto 'DRIVE_ROOT_FOLDER_ID' en Streamlit."
    try:
        info_cuenta = dict(st.secrets["gcp_service_account"])
        credenciales = ServiceAccountCredentials.from_service_account_info(info_cuenta, scopes=DRIVE_SCOPES)
        servicio = build("drive", "v3", credentials=credenciales, cache_discovery=False)
        return servicio, None
    except Exception as e:
        return None, f"No se pudo inicializar la conexión con Google Drive: {e}"


def drive_listar_carpetas(servicio, parent_id):
    query = f"'{parent_id}' in parents and mimeType='{MIME_CARPETA}' and trashed=false"
...
def drive_listar_archivos(servicio, parent_id):
    query = f"'{parent_id}' in parents and mimeType!='{MIME_CARPETA}' and trashed=false"
    resultados = servicio.files().list(
        q=query, fields="files(id, name, mimeType, size, modifiedTime)", orderBy="name", pageSize=1000,
        supportsAllDrives=True, includeItemsFromAllDrives=True,
    ).execute()
    return resultados.get("files", [])


def drive_crear_carpeta(servicio, nombre, parent_id):
    metadata = {"name": nombre, "mimeType": MIME_CARPETA, "parents": [parent_id]}
    carpeta = servicio.files().create(body=metadata, fields="id", supportsAllDrives=True).execute()
    return carpeta.get("id")


def drive_subir_archivo(servicio, nombre_archivo, contenido_bytes, parent_id):
    ...
    metadata = {"name": nombre_archivo, "parents": [parent_id]}  # <- aquí va la carpeta destino
    archivo = servicio.files().create(body=metadata, media_body=media, fields="id", supportsAllDrives=True).execute()
    return archivo.get("id")


def drive_descargar_archivo(servicio, file_id):
...
def drive_eliminar(servicio, file_id):
...
# ==============================================================================
# PESTAÑA "Contratos" — NUBE DE DOCUMENTOS
# ==============================================================================
with pestana_contratos:
    st.subheader("Gestión de Contratos")
    st.write("Aquí puedes gestionar los contratos asociados a los servicios de Armot.")
    ...
    def mostrar_nube():
        ...
        st.title("☁️ Mi Nube de Documentos (Google Drive)")

        servicio, error_drive = obtener_servicio_drive()
        if servicio is None:
            st.error(f"⚠️ No se pudo conectar con Google Drive: {error_drive}")
            with st.expander("📖 Cómo configurar la conexión con Google Drive"):
                st.markdown(
                    "1. En [Google Cloud Console](https://console.cloud.google.com/), crea (o usa) un proyecto "
                    "y habilita la **Google Drive API**.\n"
                    "2. Crea una **cuenta de servicio** y genera una llave en formato JSON.\n"
                    "3. En tu Google Drive, crea (o elige) la carpeta que servirá de raíz de 'Mi Nube' y "
                    "**compártela** con el correo de la cuenta de servicio (campo `client_email` del JSON), "
                    "dándole permiso de **Editor**.\n"
                    "4. Copia el ID de esa carpeta (está en la URL: "
                    "`https://drive.google.com/drive/folders/`**`ESTE_ID`**).\n"
                    "5. En `Settings → Secrets` de tu app en Streamlit Cloud (o en tu archivo local "
                    "`.streamlit/secrets.toml`), agrega:\n"
                )
                st.code(
                    'DRIVE_ROOT_FOLDER_ID = "el_id_de_tu_carpeta"\n\n'
                    '[gcp_service_account]\n'
                ...
                st.caption("Todos esos valores (excepto DRIVE_ROOT_FOLDER_ID) se copian tal cual del JSON descargado.")
            return

        root_id = st.secrets["DRIVE_ROOT_FOLDER_ID"]

        # --- 1. GESTIÓN DE CARPETAS ---
        st.sidebar.header("⚙️ Gestión de Carpetas")

        try:
            subcarpetas_existentes = drive_listar_carpetas(servicio, root_id)
        except HttpError as e:
            st.error(f"No se pudieron listar las carpetas de Drive: {e}")
            return
        nombres_a_id = {c["name"]: c["id"] for c in subcarpetas_existentes}
        nombres_carpetas = sorted(nombres_a_id.keys())

        # Todos los usuarios pueden crear carpetas
        with st.sidebar.expander("➕ Crear Nueva Carpeta"):
            nombre_nueva_carpeta = st.text_input("Nombre de la carpeta:")
            if st.button("Crear Carpeta"):
                nombre_limpio = nombre_nueva_carpeta.strip()
                if nombre_limpio != "":
                    if nombre_limpio in nombres_a_id:
                        st.warning("Esa carpeta ya existe.")
                    else:
                        try:
                            drive_crear_carpeta(servicio, nombre_limpio, root_id)
                            st.success(f"Carpeta '{nombre_limpio}' creada.")
                            st.rerun()
                        except HttpError as e:
        ...
        # RESTRICCIÓN DE ROL: Solo la sección de eliminar se muestra si el rol es 'admin'
        if rol == "admin":
            if nombres_carpetas:
                with st.sidebar.expander("🗑️ Eliminar Carpeta (Solo Admin)"):
                    carpeta_a_borrar = st.selectbox("Selecciona carpeta a borrar:", nombres_carpetas)
                    confirmar_borrado = st.checkbox("Confirmar eliminación")
                    if st.button("Eliminar carpeta"):
                        if confirmar_borrado:
                            try:
                                drive_eliminar(servicio, nombres_a_id[carpeta_a_borrar])
                                st.success(f"Carpeta '{carpeta_a_borrar}' eliminada.")
                                st.rerun()
                            except HttpError as e:
                                st.error(f"No se pudo eliminar la carpeta: {e}")
                        else:
        ...
        st.sidebar.header("📁 Directorios")
        opciones_carpetas = ["Raíz"] + nombres_carpetas
        carpeta_seleccionada = st.sidebar.selectbox("Carpeta activa:", opciones_carpetas)

        carpeta_id_actual = root_id if carpeta_seleccionada == "Raíz" else nombres_a_id[carpeta_seleccionada]

        # --- 3. CARGA DE ARCHIVOS ---
        st.sidebar.markdown("---")
        st.sidebar.header("📤 Cargar Archivos")
        archivo_subido = st.sidebar.file_uploader(
            f"Subir archivo a '{carpeta_seleccionada}'",
            type=["pdf", "xlsx", "docx"]
        )

        if archivo_subido is not None:
            nombre_archivo_seguro = Path(archivo_subido.name).name
            try:
                drive_subir_archivo(servicio, nombre_archivo_seguro, archivo_subido.getvalue(), carpeta_id_actual)
                st.sidebar.success(f"Guardado en Drive: '{nombre_archivo_seguro}'")
                st.rerun()
            except HttpError as e:
                st.sidebar.error(f"No se pudo subir el archivo: {e}")

        # --- 4. EXPLORADOR DE ARCHIVOS Y ELIMINACIÓN RESTRINGIDA ---
        st.subheader(f"📂 Contenido de: `{carpeta_seleccionada}`")
        try:
            archivos_drive = drive_listar_archivos(servicio, carpeta_id_actual)
        except HttpError as e:
            st.error(f"No se pudieron listar los archivos: {e}")
            archivos_drive = []
        archivos_drive = [a for a in archivos_drive if a["name"].lower().endswith((".pdf", ".xlsx", ".docx"))]

        if not archivos_drive:
            st.info("No hay archivos PDF, Excel o Word en esta carpeta.")
        else:
            for archivo in archivos_drive:
                ...
                with st.expander(f"{icono} — {nombre_archivo}"):
                    # Ajustar columnas según si el usuario es Admin o no
                    if rol == "admin":
                    ...
                    try:
                        bytes_archivo = drive_descargar_archivo(servicio, file_id)
                    except HttpError as e:
                        st.error(f"No se pudo descargar '{nombre_archivo}': {e}")
                        continue

                    # Botón de Descargar (Para todos los usuarios)
                    ...
                    # RESTRICCIÓN DE ROL: Botón de Eliminar Archivo (Solo visible y ejecutable por 'admin')
                    if rol == "admin":
                        if col2.button("🗑️ Eliminar", key=f"del_{file_id}"):
                            try:
                                drive_eliminar(servicio, file_id)
                                st.success(f"Archivo '{nombre_archivo}' eliminado.")
                                st.rerun()
                            except HttpError as e:
                    ...
                    with col3:
                        if extension == "xlsx":
                            try:
                                df_preview = pd.read_excel(io.BytesIO(bytes_archivo))
                                st.caption("Previsualización de datos (Excel):")
                                st.dataframe(df_preview.head(5), use_container_width=True)
                            except Exception as e:
                                st.error(f"Error al leer Excel: {e}")

                        elif extension == "pdf":
                            st.caption("Visualizador de PDF integrador:")
                            st.markdown(
                                f'<iframe src="data:application/pdf;base64,{base64.b64encode(bytes_archivo).decode("ascii")}" width="100%" height="350" type="application/pdf"></iframe>',
                                unsafe_allow_html=True
                            )

                        elif extension == "docx":
                            st.caption("📌 Previsualización rápida no disponible para Word. Utiliza el botón de descarga.")
