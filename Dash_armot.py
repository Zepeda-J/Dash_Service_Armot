...
def _normalizar_info_cuenta(info):
    """Normaliza credenciales de Google provenientes de Streamlit Secrets o JSON."""
    # Streamlit puede devolver un diccionario, un bloque TOML anidado o incluso
    # una cadena que contiene el JSON completo de la cuenta de servicio.
    if info is None:
        return {}
    ...
    # Algunas configuraciones pueden guardar el JSON dentro de una llave.
    for llave in ("gcp_service_account", "service_account", "credentials", "json"):
        if llave in info and isinstance(info[llave], (dict, str)):
            anidado = _normalizar_info_cuenta(info[llave])
            if anidado.get("client_email") and anidado.get("private_key"):
                info = anidado
                break
    ...
    # Evita que una cuenta mal formada se considere válida.
    requeridos = ("client_email", "private_key", "token_uri")
...
def _obtener_lista_credenciales():
    """
    Obtiene credenciales de Google de forma robusta.

    Soporta:
      1) [gcp_service_account] en Streamlit Secrets.
      2) [gcp_service_account_2] en Streamlit Secrets.
      3) [connections.gsheets] si contiene credenciales de servicio.
      4) Credenciales planas en st.secrets (client_email/private_key).
      5) Un secreto que contenga el JSON completo como texto.
      6) Archivos *.json locales de cuentas de servicio.
    """
    credenciales_lista = []

    def agregar(info):
        info = _normalizar_info_cuenta(info)
        if info.get("client_email") and info.get("private_key"):
            firma = (info.get("client_email"), info.get("private_key"))
            if not any(
                (x.get("client_email"), x.get("private_key")) == firma
                for x in credenciales_lista
            ):
                credenciales_lista.append(info)

    # 1. Llaves estándar de Streamlit Secrets.
    for secret_key in ("gcp_service_account", "gcp_service_account_2"):
        try:
            if secret_key in st.secrets:
                agregar(st.secrets[secret_key])
        except Exception:
            pass

    # 2. Configuración [connections.gsheets].
    try:
        if "connections" in st.secrets:
            connections = st.secrets["connections"]
            if "gsheets" in connections:
                agregar(connections["gsheets"])
    except Exception:
        pass

    # 3. Caso en que el usuario pegó directamente las llaves en Secrets,
    #    sin crear la sección [gcp_service_account].
    try:
        claves_credenciales = {
        ...
        for clave in claves_credenciales:
            try:
                if clave in st.secrets:
                    planas[clave] = st.secrets[clave]
            except Exception:
                pass
        if planas.get("client_email") and planas.get("private_key"):
            agregar(planas)
    except Exception:
        pass

    # 4. Buscar secretos que contengan directamente un JSON de service account.
    try:
        for clave in st.secrets:
            try:
    ...
    # 5. Archivos JSON locales. Se busca junto al script, no en un directorio
    #    de trabajo que pueda cambiar al ejecutar Streamlit.
    for ruta_json in BASE_DIR.glob("*.json"):
        try:
...
def obtener_clientes_gspread():
    ...
    if not lista_info:
        return [], (
            "No se encontraron credenciales válidas de Google. "
            "En Streamlit Cloud usa [gcp_service_account] con client_email y private_key, "
            "o coloca el JSON de la cuenta de servicio junto al script en local."
        )
    ...
    for info in lista_info:
        try:
            ...
            clientes.append(
                (cliente, info.get("client_email", "Email desconocido"))
            )
        except Exception as e:
            errores.append(
                f"Error con {info.get('client_email', 'cuenta desconocida')}: {e}"
            )

    if not clientes:
        return [], (
            "No se pudo autenticar con ninguna cuenta de servicio: "
            + "; ".join(errores)
        )
    ...
    def get_drive_service():
        ...
        if not lista_info:
            st.error(
                ...
                "cuenta de servicio en la misma carpeta del script."
            )
            return None
        for info in lista_info:
            try:
